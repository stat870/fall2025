...
The experiment consists of 3 groups of cows 
...
$$\eta_{ijk} = \mu_0 + D_i + f(t_{ij}) + b_{k},$$
...
```{r}
m1 <- gam(GHG_flux ~ diet + s(group, bs = "re") + 
            s(doy, bs = "tp", k = 35), data = dat,
          family = Gamma(link = "log"))

dat_plot <- expand.grid(doy = seq(min(dat$doy), max(dat$doy), length.out = 1000), 
                        diet = unique(dat$diet)) %>% 
  mutate(group = case_when(doy <95 ~ 1, 
                           between(doy, 97, 121) ~ 2, 
                           between(doy, 122, 151.4) ~ 3, 
                           between(doy, 151.5, 177.8) ~ 4, 
                           doy>178 ~ 5, 
                           TRUE ~ 6)) %>% 
...
dat_plot %>% 
  ...
  geom_ribbon(aes(group = paste(diet, group),
                  fill = diet,
                  ymin = exp(pred_m1 - se_m1) , ymax = exp(pred_m1 +se_m1)), 
              alpha = .3)+
