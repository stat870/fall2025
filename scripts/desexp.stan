...
parameters {
  ...
  real alpha_zi;                        // Intercept for the zero-inflation probability (logit scale)
...
transformed parameters {
  // Probability of excess zero (on logit scale)
  vector[N] logit_phi = rep_vector(alpha_zi, N);
...
model {
  ...
  alpha_zi ~ normal(0, 2);          // Prior for logit(phi)
  ...
  for (n in 1:N) {
    ...
    if (behavior_count[n] == 0) {
      ...
      target += log_sum_exp(bernoulli_logit_lpmf(1 | logit_phi[n]),
                            bernoulli_logit_lpmf(0 | logit_phi[n]) + poisson_log_lpmf(0 | log_lambda[n]));
    } else {
      // P(y>0) = (1-phi) * P(Poisson(lambda)=y)
      target += bernoulli_logit_lpmf(0 | logit_phi[n]) + poisson_log_lpmf(behavior_count[n] | log_lambda[n]);
    }
  }
}
